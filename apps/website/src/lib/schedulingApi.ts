...
// Check if helper is available for a specific event time
export async function isHelperAvailableForEvent(
  ...
  if (USE_MOCK_DATA) {
    const { isHelperAvailableForEvent: mockIsHelperAvailableForEvent } =
      await import('./mock/helpers.js');
    return mockIsHelperAvailableForEvent(helperId, eventStart, eventEnd);
  }
